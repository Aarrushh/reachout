...
Target layout (Madrid, 5 stages — see CONTEXT.md):
Stage 01  parse-query     AGENTIC   free text  -> SearchIntent (validated)
Stage 02  geo-resolve     HARDCODED location   -> shops in radius   [pending: scripts/geo_resolve.py]
Stage 03  match-and-ping  HARDCODED SearchIntent -> ranked matches + pings
Stage 04  format-results  AGENTIC   matches    -> ranked shop list (optional AI)
Stage 05  map-render      HARDCODED ranked list -> GeoJSON          [pending: scripts/map_render.py]

Until the execution phase lands stages 02 and 05, this orchestrator walks
01 -> 03 -> 04 on the legacy data.
...
def run(query, lat, lng, radius_km=5.0, use_llm=False):
    ...
    _write_output("03-match-and-ping", "matches.json", result)
    print(f"           matches       : {result['match_count']}")

    print("Stage 04  format-results : building consumer card")
    card = format_card(result, use_llm=use_llm)
    ok, err = v.validate(card, "shop_match.schema.json")
    if not ok:
        print("           [WARN] card failed schema:", err)
    _write_output("04-format-results", "card.json", card)
