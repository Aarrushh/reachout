...
> **Corrected 2026-08-16 during Task 7.** This paragraph first read 580 / 78 / 480: it subtracted all 23 flips as demotions and then added the rescue a second time, when the rescue was already one of the 23. The live run after Task 5 measured 581 / 77 / 481, and re-scoring the same 658 rows against `relevance.py` as of 4a41fb0 reproduces the 602 / 56 / 503 baseline exactly. The arithmetic was wrong, not the code.
>
> **Final state, after the final-review fix round.** Moving `se` out of the head tokens and into the `se puede` phrase marker demotes one further row (`los huevos se pueden congelar`, genuinely informational), so the branch ships **580 commercial / 78 noise / 480 commercial clusters** — the same three numbers this paragraph originally guessed, reached by a different route. The two other scorer changes in that round (removing `sin`/`sobre`/`a` from the stopwords, and refusing chain names an exemption from the head rule) move no live row.
