...
export function buildYScalesForLines({
   ...
   * bar charts (e.g. StockOutRiskChart) resolve their domain through this
   * function, not through BarChart's own top-level `valueScale` — so
   * `domainMax`'s `nice: false` has to reach here too, or a pinned top
   * (e.g. 100) could still get rounded outward by d3's `.nice()`. Default
   * `true` preserves every existing call site's behaviour unchanged. */
  nice = true,
}: {
  lines: LineConfig[];
