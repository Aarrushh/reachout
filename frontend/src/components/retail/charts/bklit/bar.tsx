...
/* D11 local patch P1 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4 */
...
export interface BarProps {
  ...
  minBarHeight?: number;
  /** D11 local patch P1 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4.
   * Per-datum fill override, e.g. colouring each bar by the server's
   * `direction` field (rising/falling/flat) rather than one static `fill`
   * for the whole series. Takes precedence over `fill` when it returns a
   * value; falls back to `fill` per-datum otherwise. */
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        // D11 local patch P1 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4.
        // Per-datum fill: colorAccessor wins when provided, else the
        // series-wide `fill` prop (unchanged upstream behaviour).
        const datumFill = colorAccessor ? colorAccessor(d, i) : fill;
