...
/* D11 local patch P2 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4 */
...
import { cn } from "@/components/retail/charts/bklit/lib/utils";
...
export interface BarChartProps {
  ...
  status?: ChartStatus;
  /** D11 local patch P2 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4.
   * Pins the value-axis domain top instead of deriving it from the data
   * (`maxValue * 1.1`). When set, the scale is built with `nice: false` so
   * the pinned top renders exactly as given — e.g. `domainMax={100}` always
   * puts the top gridline/tick at 100, never rounded by d3's `.nice()`. */
  domainMax?: number;
}
...
const ChartCore = memo(function ChartCore({
  ...
  // D11 local patch P2 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4. When
  // `domainMax` is set it replaces the data-derived `maxValue * 1.1` top,
  // and `nice: false` keeps that pinned top from being rounded outward.
  const valueScale = useMemo(() => {
    const range = isHorizontal ? [0, innerWidth] : [innerHeight, 0];
    return scaleLinear({
      range,
      domain: [0, domainMax ?? maxValue * 1.1],
      nice: domainMax === undefined,
    });
  }, [innerWidth, innerHeight, maxValue, isHorizontal, domainMax]);
  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      data,
      innerHeight,
      // D11 local patch P2 — see docs/IMPLEMENTATION_PLAN_V3.md §3.4. Vertical
      // orientation (e.g. StockOutRiskChart) resolves its domain here, not
      // through `valueScale` above, so `domainMax` has to be threaded through
      // this path too for the pinned top to actually render.
      resolveDomain: (dataKeys) => {
        ...
        return [0, domainMax ?? (max || 100) * 1.1];
      },
      nice: domainMax === undefined,
    });
  }, [data, innerHeight, isHorizontal, lines, valueScale, domainMax]);
